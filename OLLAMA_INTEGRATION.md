...
### P2（2026-09-22 上线，经 Grill Me 对齐：核心两项 / 生成5句→选1练+可入库 / 评分独立开关默认关+手动按钮）
- **④ 自适应生成**：练习页操作栏加「🎲 生成练习句」按钮 → 调 `generate`（按当前难度 level + 最弱薄弱类别 category）生成 5 句；面板每句可「练这道」（替换当前题成为可练新句）或「存入题库」（调 `addCustomSentence` 加入自定义句库并进入 SR 循环）。
- **⑥ 可选 AI 评分**：工具栏新增独立开关「📊 AI 评分（可选）」**默认关**；开启且提交/揭示答案后，答案区出现「📊 给当前句评分」按钮，手动点才调 `grade` 显示 判定/分数/修正句/逐条错误/鼓励评语。**不写入 SR / 掌握度**（仅辅助参考）。
- `coach.js`：`window.Coach` 新增 `isGradeEnabled` / `setGradeEnabled`（持久化 `sl_ai_grade`，默认 false）。
- `refreshAiButtons`：生成按钮随 AI 可用性禁用；评分按钮额外受评分开关门控。
- 说明：真正的 SRS 写入受规划第 8 节约束（薄弱点不污染间隔重复），故 P2 未做「看板→SRS」硬联动；「看板→开练动作面」已在 P1 覆盖。

### 第三期总结（P0 + P1 + P2）
- 完整闭环：提示梯 → 拆解 → FSI Drills → 写错补强 → 薄弱点看板开练 → 自适应生成新题 → 可选 AI 评分（纯参考）。
- 全部 AI 能力遵循：开关关 / Ollama 未连时自动禁用，离线降级不影响基础手写造句练习。
